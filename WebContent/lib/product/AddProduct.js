Ext.define('Pcbms.product.AddProduct', {
	extend : 'Ext.form.Panel',
	alias : "widget.prodadd",
	bodyPadding : 10,
	bcbh : "",
	createBcbh : function() {// 生成工程编号
		...
		Ext.each(this.getForm().getFields().items, function(item) {
			...
			if (item.getName().indexOf("projectFile.") < 0) {
				return;
			}
			...
			if (item.getXType() == "radiofield") {
				if (item.getValue() && item.getName() == "projectFile.bcbhtype") {
		...
		Ext.Ajax.request({
					...
					success : new Pcbms.ajaxHandler({
								success : function(str) {
									me.bcbh = str.bcbh;
									me.getForm().findField("projectFile.bcbh")
											.setValue(me.bcbh);
								},
								error : function(r) {
...
{columnWidth: .5,defaults:{
	xtype:"textfield",
	labelWidth : 60,
	anchor:"95%",
	allowBlank:false
},layout:"anchor",border:false,items:[{
