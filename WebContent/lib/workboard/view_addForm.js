 ...
 * 组工作版
 */
Ext.define('Pcbms.product.WorkBoardInputForm', {
	extend : 'Ext.form.Panel',
	alias : "widget.wbaddform",
	layout : "border",
	border : false,
	bodyPadding : 5,
	...
	submitForm : function() {
		...
		if (me.getForm().isValid()) {
			...
			for (var i = 0; i < numgrid.store.data.items.length; i++) {
				...
				params["workBoardInfo.productList[" + i + "].prodId"] = line.data["prodId"];
				params["workBoardInfo.productList[" + i + "].pbnum"] = line.data["num"];
			}
			// 获取工序参数列表
			...
			for (var i = 0; i < stepgrid.store.data.items.length; i++) {
				...
				for (var j = 0; j < line.data.stepParams.length; j++) {
					...
					params["workBoardInfo.stepList[" + i + "].stepParams[" + j
							+ "].id"] = lin["id"];
					params["workBoardInfo.stepList[" + i + "].stepParams[" + j
			...
			// 层压结构数据
			params["workBoardInfo.stratification"] = me.down("#cengyagrid")
					.getValue();
			me.getForm().submit({
	...
	initComponent : function() {
		...
		me.stepStore = Ext.create("Ext.data.Store", {
			model : "Step",
		...
		Ext.applyIf(me, {
			items : [{
				...
				columns : [{
							...
							text : '交货日期',
							renderer : function(v) {
								return Ext.util.Format.dateRenderer('Y-m-d')(new Date(v));
							}
						}, {
							xtype : 'numbercolumn',
							width : 120,
				...
				tbar : [{
							...
							handler : function(b) {
								...
								if (index == null) {
									...
									index = grid.store.indexOf(index) - 1;
								}
								if (index < 0) {
									index = 0;
								}
